...
export type ChannelSessionRef =
  ...
  | { provider: "waconector"; waconector_instance_name: string };
...
export const CHANNEL_SESSION_REF_COLUMNS =
  "provider, waha_session_name, meta_phone_number_id, zernio_account_id, waconector_instance_name";
...
export function resolveSessionRef(session: ChannelSessionRef): string {
  switch (session.provider) {
    ...
    // Nome da instância no provider não-oficial (EvoAPI, Z-API, ...). No piloto
    // de instância única via env, é informativo — o adapter resolve a instância
    // pelo apiKey, não pelo nome.
    case "waconector":
      return session.waconector_instance_name;
  }
}
